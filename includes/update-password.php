...
if (isset($_POST['Change Passowrd'])) {
    $Password = md5($_POST['opwd']);
    //$useremail = $_SESSION['login'];
    $newpassword = md5($_POST['npwd']);
    $sql = mysqli_query($conn, "SELECT Password FROM tblbooking where id='$id'");
    $num = mysqli_fetch_array($sql);
    if ($num > 0) {
        $con = mysqli_query($conn, "update tblbooking set Password='$newpassword' where id='$id'");
    ...
    <form class="w3-container" action="" method="post" name="login_form" id="login_form">
        ...
        <div class=" w3-section">
            ...
            <div>
                ...
                <input class="w3-input w3-border w3-margin-bottom" type="password" placeholder="Enter email" name="opwd"
                    id="opwd" value="<?php echo $results['Password']; ?>" required>
            </div>
            <div>
